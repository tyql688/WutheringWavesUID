...
from gsuid_core.plugins.WutheringWavesUID.WutheringWavesUID.utils.resource.RESOURCE_PATH import GUIDE_CONFIG_MAP
...
class GuideTap:
    ...
    async def get_bbs_list(self, auther_id: int):
    ...
    async def get_guide_all_moment(self, auther_id: int):
        response = await self.get_bbs_list(auther_id)
        soup = BeautifulSoup(response.text, 'html.parser')
        # 查找所有匹配的span标签
        span_tags = soup.find_all("span", {"class": "moment-article__summary--title paragraph-m16-w16 font-bold"})
        # 查找所有匹配的a标签
        a_tags = soup.find_all("a", {"class": "tap-router moment-article__image-list"})
    ...
    async def get_ann_detail_pic(self, href: str, auther_id: int):
        ...
        image_urls = [img_tag["src"] for div_tag in div_tags if (img_tag := div_tag.find("img")) and img_tag.get("src")]
