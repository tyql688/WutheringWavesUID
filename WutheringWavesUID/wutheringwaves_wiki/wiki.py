...
async def draw_wiki_weapon(raw_data: Dict):
    ...
    # 武器信息
    weapon_data = next(
        (i for i in content["modules"] if i["title"] == "武器信息"), None
    )
    if not weapon_data:
        return "暂无该武器wiki"
    # 突破材料
    upgrade_data = next(
        (i for i in content["modules"] if i["title"] == "突破材料"), None
    )
    # 基础信息
    base_data = next(
        (i for i in weapon_data["components"] if i["title"] == "基础信息"), None
    )
    # 武器描述
    weapon_detail = next(
        (i for i in weapon_data["components"] if i["title"] == "武器描述"), None
    )
    # 角色统计
    weapon_statistics = next(
        (i for i in weapon_data["components"] if i["type"] == "tabs-component"), None
    )
    # 突破材料
    upgrade_detail_data = next(
        (i for i in upgrade_data["components"] if i["title"] == "突破材料"), None
    )
    ...
    await parse_weapon_statistic_content(
        weapon_statistics["tabs"][-1]["content"], weapon_image
    )
...
async def parse_weapon_base_content(content, image, card_img):
    ...
    # 提取“稀有度”
    rarity_pic = (
        soup.find("td", text="稀有度").find_next_sibling("td").find("img")["src"]
    )
...
async def parse_weapon_material_content(content, card_img):
    ...
    while next_td:
        ...
        for idx, img in enumerate(imgs):
            material = Image.open(BytesIO((await sget(img["src"])).content)).convert(
                "RGBA"
            )
...
async def draw_wiki_char(raw_data: Dict, query_role_type: str):
    ...
    # 角色组件
    role_data = next(
        (i["role"] for i in base_data["components"] if i["type"] == "role-component"),
        None,
    )
    # 角色统计
    char_statistics = next(
        (i for i in base_data["components"] if i["type"] == "tabs-component"), None
    )

    # 共鸣链
    mz_data = next(
        (i for i in char_data["components"] if i["type"] == "basic-component"), None
    )
    if not mz_data:
        mz_data = next(
            (i for i in base_data["components"] if i["title"] == "共鸣链"), None
        )
    # 技能介绍
    skill_data = next(
        (i for i in char_data["components"] if i["title"] == "技能介绍"), None
    )
    if not skill_data:
        skill_data = next(
            (i for i in base_data["components"] if i["title"] == "技能介绍"), None
        )

    char_pic = Image.open(
        BytesIO((await sget(role_data["figures"][0]["url"])).content)
    ).convert("RGBA")
    char_pic = char_pic.resize((600, int(600 / char_pic.size[0] * char_pic.size[1])))

    char_bg = Image.open(
        BytesIO((await sget(role_data["backgroundImage"])).content)
    ).convert("RGBA")
    ...
    # 角色统计信息
    role_statistic_pic = await parse_role_statistic_content(
        char_statistics["tabs"][-1]["content"]
    )
    ...
    if query_role_type == "天赋":
        ...
        for index, tab in enumerate(skill_data["tabs"]):
            ...
            skill_image = Image.open(
                BytesIO((await sget(skill_image)).content)
            ).convert("RGBA")
...
async def parse_mz_content(content):
    ...
    for row in rows[1:]:  # 跳过标题行
        ...
        temp = {
            ...
            "effect": [
                collect_text_and_style(
                    columns[1],
                    parent_font_size=24,
                    is_force_parent_font_size=True,
...
def draw_html_text2(
    ...
    def draw_my_text(
        segments_in_lines, x_offset, y_offset, draw, max_line_width, is_draw=True
    ):
    ...
    if not image:
        y_offset = draw_my_text(
            segments_in_lines,
            padding[0] + init_padding[0],
            padding[1] + init_padding[1],
            draw,
            max_line_width,
            False,
        )
