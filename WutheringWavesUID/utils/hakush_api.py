...
async def download_one_avatar_pic(
    role_detail_temp, resource_id, is_force: bool = False
):
    ...
    await download(
        url, AVATAR_PATH, name, tag=f"[鸣潮->【{role_name}-{resource_id}】->头像]]"
    )
...
async def download_one_pile_pic(role_detail_temp, resource_id, is_force: bool = False):
    ...
    if not is_force and path.exists():
        return
    resource_path = (
        role_detail_temp["Background"].split(".")[0].replace("/Game/Aki/", "")
    )
    url = f"{HAKUSH_MAIN_URL}/{resource_path}.webp"
    await download(
        url, ROLE_PILE_PATH, name, tag=f"[鸣潮->【{role_name}-{resource_id}】->立绘]"
    )
...
async def download_one_skills(role_detail_temp, resource_id, is_force: bool = False):
    ...
    for skill_id, skill in skill_tree.items():
        ...
        await download(
            url, skill_pic_dir, name, tag=f"[鸣潮->【{role_name}-{resource_id}】->技能]"
        )
...
async def download_one_chains(role_detail_temp, resource_id, is_force: bool = False):
    ...
    for chain_id, chain in chains.items():
        ...
        await download(
            ...
            chain_pic_dir,
            name,
            tag=f"[鸣潮->【{role_name}-{resource_id}】->共鸣链]",
        )
...
async def download_all_weapon_pic(all_weapon: Dict, is_force: bool = False):
    for resource_id, temp in all_weapon.items():
        ...
        await download(
            url, WEAPON_PATH, name, tag=f"[鸣潮-【{weapon_name}-{resource_id}】->武器]"
        )
